...
import 'package:myapp/widgets/mago_card.dart'; // Importe o MagoCard

class ListaDeMagosScreen extends StatefulWidget {
  const ListaDeMagosScreen({super.key});

  @override
  State<ListaDeMagosScreen> createState() => _ListaDeMagosScreenState();
}

class _ListaDeMagosScreenState extends State<ListaDeMagosScreen> {
  ...
  late List<Mago> magos;

  @override
  void initState() {
    super.initState();
    final Map<String, dynamic> decodedJson = jsonDecode(jsonData);
    final List<dynamic> magoList = decodedJson['magos'];
    magos = magoList.map((m) => Mago.fromJson(m)).toList();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Magos do Mundo'),
      ),
      body: ListView.builder(
        itemCount: magos.length,
        itemBuilder: (context, index) {
          final mago = magos[index];
          return MagoCard(mago: mago); // Usando o widget MagoCard
        },
      ),
    );
  }
